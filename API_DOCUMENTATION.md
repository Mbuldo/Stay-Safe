...
Get the full question bank used by the assessment engine. **Requires authentication.**

#### GET /api/assessments/questions/:category

Get questions for one category (for dynamic assessment forms). **Requires authentication.**
...
Check whether live AI is configured. **Requires authentication.**
...
- `/api/articles` now includes a seeded baseline library and periodic MedlinePlus-synced content.
- `/api/resources` is seeded with Nairobi-focused support centers and hotlines.
